...
Variational Inference approximates the posterior with a simpler distribution $q(\bm{\beta}, \tau_e)$ from a tractable family. We use the mean-field factorization:
\begin{equation}
...
since $\KL(q \| p) \geq 0$. Maximizing the ELBO minimizes the reverse KL divergence $\KL(q \| p)$.
...
We parameterize $\bm{\Sigma}_\beta$ via Cholesky decomposition $\bm{\Sigma}_\beta = \bm{L}\bm{L}^T$ to maintain positive definiteness, or work directly with precision $\bm{\Lambda}_\beta = \bm{\Sigma}_\beta^{-1}$.
...
where $\bm{H} = \nabla^2 \ELBO$ is the Hessian matrix. For maximization, we use $-\bm{H}^{-1}$ to ascend.
...
\item \textbf{Safeguard}: Add regularization $\bm{H} + \lambda \bm{I}$ or use trust region
\end{itemize}
...
and $\bm{B}_t \approx -\bm{H}^{-1}$ (inverse Hessian for maximization).
...
\begin{equation}
\text{maximize} \quad \ELBO(\bm{\phi}) - \rho \sum_{j=1}^{p} \max(0, \sigma^2_{\min} - \sigma^2_{\beta_j})^2
\end{equation}
...
\item \textbf{Newton}: Fastest convergence near optimum but expensive per iteration and may need regularization
...
\item Add full Hessian computation and Newton's method with regularization
