...
\vspace{2em} , a well-known failure mode of mean-field VI is \emph{posterior variance collapse}: the optimised approximation $q(\bm{\theta})$ is systematically overconfident, underestimating posterior uncertainty. This is a direct consequence of the KL direction minimised during ELBO maximisation, compounded by the mean-field factorisation assumption. See Appendix~B for the full mechanistic explanation.

This project addresses variance collapse via two complementary strategies. The first applies new optimisation methods to the \emph{standard} ELBO objective: gradient ascent, Newton's method, and BFGS are compared to determine whether the choice of optimiser affects dispersion quality. The second modifies the objective function itself, augmenting the ELBO with an entropy regularisation term $\lambda H[q(\bm{\beta})]$ to create a new objective $\mathcal{L}_{\mathrm{reg}}(\bm{\phi}) = \mathcal{L}(\bm{\phi}) + \lambda H[q(\bm{\beta})]$ that explicitly incentivises broader posteriors. Both the optimiser-comparison and modified-ELBO strategies are explored.

Throughout all gradient-based methods, the variational covariance $\bm{\Sigma}_\beta$ must remain symmetric positive definite at every iteration.\footnote{This structural constraint is satisfied automatically via Cholesky reparameterisation; see Appendix~A.} Standard gradient steps in Euclidean space can violate this requirement, leading to numerical failure. The solution is to parameterise the optimisation over the Cholesky factor $\bm{L}$ rather than $\bm{\Sigma}_\beta$ directly.
...
\item $\bm{\Sigma}_\beta \in \R^{p \times p}$: posterior covariance\footnote{\textbf{Covariance} describes how much two variables move together. $\Sigma_{\beta,ij}$ tells you how much component $\beta_i$ and $\beta_j$ of the regression coefficients tend to vary together in the posterior. The diagonal entries $\Sigma_{\beta,ii}$ are the variances (spread) of each individual coefficient.} (symmetric positive definite\footnote{A matrix $\bm{A}$ is \textbf{positive definite} if $\bm{v}^T\bm{A}\bm{v} > 0$ for every non-zero vector $\bm{v}$. Intuitively: the matrix amplifies every direction --- there is no direction in which it squashes things to zero or flips them negative. For a covariance matrix this means every direction in parameter space has genuine positive uncertainty. Written $\bm{A} \succ 0$.})
...
\subsection*{Phase 1: Mathematical Foundations — COMPLETE}

\textbf{Status: All mathematical derivations and implementation specifications verified and documented.}
...
\item Store data in NumPy arrays for efficient computation
\end{itemize}

\item Baseline Implementation: CAVI (40\%)
\begin{itemize}
\item Implement closed-form coordinate updates
\item Track ELBO convergence
\item Stopping criterion: $|\ELBO^{(t+1)} - \ELBO^{(t)}| < 10^{-6}$
\item Numerical gradient check against analytical CAVI updates
\end{itemize}

\item Method 1: Gradient Ascent (40\%)
\begin{itemize}
\item Implement ELBO gradient computation with Cholesky and log-space transforms
\item Line search: Armijo backtracking (Algorithm 1 in Implementation Specifications)
\item Stopping criterion: $\|\nabla \ELBO\| < 10^{-6}$ or relative ELBO change $< 10^{-8}$
\item Track iteration count and ELBO values
\end{itemize}

\item Method 2: Newton's Method (25\%)
\begin{itemize}
\item Implement Hessian computation (block structure)
\item Regularisation: add $\lambda \bm{I}$ with $\lambda = \max(10^{-6}, -2\lambda_{\min})$ if Hessian not negative definite
\item Line search for step size $\alpha_t$
\item Track convergence and computational cost
\end{itemize}

\item Method 3: BFGS Quasi-Newton (30\%)
\begin{itemize}
\item Initialise inverse Hessian approximation: $\bm{B}_0 = \bm{I}_d$ 
\item Implement BFGS update with curvature condition check
\item Track superlinear convergence
\end{itemize}

\item Comparison and Analysis (20\%)
\begin{itemize}
\item Compare convergence speed (iterations to $10^{-6}$ tolerance)
\item Compare computational cost (time per iteration, total time)
\item Compare solution quality (posterior approximation accuracy vs MCMC)
\item Analyse when each method performs best
\item Generate convergence plots and performance tables
\end{itemize}
\end{enumerate}
...
\textbf{Immediate coding priority (May 2026):}
\begin{itemize}
  \item[\textbf{!}] \textbf{Demonstrate variance collapse first} --- run CAVI and Gibbs on the same synthetic dataset and plot the SD ratio (VB $\sigma$ / Gibbs $\sigma$) for each parameter. This confirms the collapse phenomenon empirically, establishes the Gibbs sampler as ground-truth baseline, and provides the motivating figure for the final report. The classes already exist (\texttt{SimpleLinearVB}, \texttt{SimpleLinearGibbs} in \texttt{vb\_algorithms\_py.py}); only a driver script and plot are needed.
  \item[\textbf{!}] \textbf{Implement reparameterisation layer} before any gradient-based method can run:
  \begin{itemize}
    \item Forward transform: $\bm{\Sigma}_\beta \to \bm{L}$ (Cholesky), $L_{ii} \to \tilde\ell_{ii} = \log L_{ii}$, $a_e \to \eta_a = \log a_e$, $b_e \to \eta_b = \log b_e$
    \item Inverse transform: recover $\bm{\Sigma}_\beta$, $a_e$, $b_e$ on every ELBO evaluation
    \item Gradient transform: chain rule $\nabla_\theta \mathcal{L}$ from $\nabla_\phi \mathcal{L}$ --- formulas in \S Implementation Specifications
    \item Hessian transform: additionally required for Newton's method
  \end{itemize}
  \item Implement gradient ascent with Armijo backtracking line search, then Newton's method and BFGS
  \item All formulas are fully derived in \texttt{20260502 status-report.tex} \S Implementation Specifications
\end{itemize}

\textbf{Week 7 (24-31 March):}
\begin{itemize}
\item Implement CAVI baseline in Python with ELBO tracking
\item Implement numerical gradient checker
\item Verify CAVI gradients against finite differences
\item Begin gradient ascent implementation
\end{itemize}

\textbf{Week 8 (1-7 April):}
\begin{itemize}
\item Complete gradient ascent with Armijo line search
\item Implement Newton's method with regularisation
\item Begin BFGS implementation
\end{itemize}

\textbf{Week 9 (8-14 April):}
\begin{itemize}
\item Complete BFGS implementation
\item Run convergence comparisons on test case
\item Generate convergence plots (ELBO vs iteration, time vs iteration)
\end{itemize}

\textbf{Week 10 (15-21 April):}
\begin{itemize}
\item Sensitivity analysis: different initialisations, data sizes
\item MCMC validation (Gibbs sampler for ground truth)
\item Performance profiling and optimisation
\end{itemize}

\textbf{Week 11 (22-28 April):}
\begin{itemize}
\item Final empirical comparison
\item Generate all figures and tables for report
\item Draft results section
\end{itemize}

\textbf{Week 12 (May):}
\begin{itemize}
\item Complete final report with performance analysis
\item Review and polish all sections
...
\subsection*{Why This Is Not in the Course Notes}

Cholesky reparameterisation is a \emph{numerical implementation technique}, not a course-level optimisation concept. The course teaches the optimisation methods (gradient ascent, Newton, BFGS); Cholesky is the domain-specific choice of how to handle the manifold structure of the constraint set $\bm{\Sigma}_\beta \succ 0$ when applying those methods to this problem. It is standard practice in variational inference and Bayesian computation.

\subsection*{Reference}

\noindent\textbf{Bishop, C.M.} (2006). \textit{Pattern Recognition and Machine Learning}. Springer.
\begin{itemize}
  \item Section 2.3 (The Gaussian Distribution) --- symmetric positive definite covariance matrices and their properties.
  \item Appendix C (Properties of Matrices) --- Cholesky factorisation and positive definite matrices.
\end{itemize}
\noindent\textit{Workspace copy available at:} \texttt{reference/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf}
...
\textbf{This is the most principled of the three approaches} and is directly amenable to the gradient-based optimisation methods (gradient ascent, Newton, BFGS) already implemented for this project.

\subsection*{Selected Approach for This Project}
