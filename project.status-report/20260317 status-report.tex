...
\item Gradient Ascent: Baseline first-order method, expected to be slowest but most robust
\item Newton's Method: Best convergence rate (quadratic) if Hessian well-conditioned
\item BFGS: ``Best general-purpose optimiser'' for smooth problems (course recommendation)
\end{itemize}
...
\item Method 1: Gradient Ascent (25\%)
\begin{itemize}
...
\item Regularisation: add $\lambda \bm{I}$ if Hessian not positive definite
...
\end{itemize}
\item Understanding: Clear explanation of why each method performs as it does
\end{enumerate}
\newpage
...
\item Mathematical derivations\footnote{This needs some kind of peer review}:   
\begin{itemize}
...
\item Draft method selection justification:  
\begin{itemize}
...
\item Hessian properties: Diagonal blocks are negative definite (ELBO is locally concave?), supporting Newton's method applicability

\item Dimensionality: With $d=11$ parameters, Hessian inversion costs may be manageable for Newton's method ( L-BFGS reduction may not be needed)

\item Parameter coupling: Mixed Hessian blocks non-zero, indicating gradient-based methods exploiting joint updates should outperform coordinate ascent
\end{itemize}
\newpage
...
\begin{enumerate}[leftmargin=*]
\item Optimisation formulation: Convert inference problem to unconstrained optimisation
\item Gradient computation: Analytical derivation and numerical implementation
\item Method implementation: Code gradient ascent, Newton, and BFGS from scratch
\item Line search: Apply bisection or Wolfe conditions for step size selection
\item Stopping criteria: Implement gradient norm and function value convergence tests
\item Performance comparison: Empirical evaluation of convergence rates and computational costs
\item Method selection: Understand when/which  optimisation algorithm is to be used
\end{enumerate}

Key insight: The VI problem represents a real computational challenge in machine learning, making the comparison of optimisation methods both  valuable and  relevant.

\section*{Timeline}

\begin{tabular}{ll}
March 17, 2026 & Mathematical derivations completed \\
March 18, 2026 & Data generation and CAVI implementation \\
March 19, 2026 & Gradient ascent and Newton's method implementation \\
March 20, 2026 & BFGS implementation and comparison analysis \\
March 20, 2026 & Progress report submission \\
\end{tabular}
...
\begin{itemize}
\item Applies to course material (gradient descent, Newton, BFGS)
\item Requires implementation of algorithms with line search and stopping criteria
\item Enables performance comparison on a realistic and meaningful problem
\item Demonstrates an application of optimisation theory to modern machine learning
\end{itemize}
