...
\section*{Progress To Date (as of 1 April 2026)}
...
\begin{itemize}
  \item[\textbf{!}] \textbf{Implement reparameterisation layer in \texttt{vb\_algorithms\_py.py}} before any gradient-based method can run:
  \begin{itemize}
    ...
    \item Inverse transform: recover $\bm{\Sigma}_\beta$, $a_e$, $b_e$ from unconstrained variables on every ELBO evaluation
    \item Gradient transform: chain rule $\nabla_\theta \text{ELBO}$ from $\nabla_\phi \text{ELBO}$ --- formulas in \S Implementation Specifications
    \item Hessian transform: additionally required for Newton's method
  \end{itemize}
  \item Implement gradient ascent with Armijo backtracking line search using $h(\alpha) = \text{ELBO}(\bm{\phi}^{(t)} + \alpha \bm{d}^{(t)})$
  \item Implement Newton's method with Hessian regularisation
  \item Implement BFGS with Wolfe conditions
  \item All formulas are fully derived and ready in \texttt{20260502 status-report.tex} \S Implementation Specifications
\end{itemize}
...
\end{equation}
This direction penalises $q$ for placing mass where $p$ is small, but \emph{does not} penalise $q$ for missing mass where $p$ is large. The result is a $q$ that is narrower (overconfident) relative to the true posterior, particularly in curved or multi-modal posteriors.
