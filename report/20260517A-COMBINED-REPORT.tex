...
The $p=3$ design matrix retains the Normal--Gamma structure intact, so all four methods
run with the same closed-form CAVI updates as Case Study~1.
CAVI converges in approximately 50 iterations; BFGS matches its final ELBO in fewer,
while gradient ascent requires more iterations to reach the same limit.
With $n=100$ the posterior is sharper than in CS1, yet all methods recover the true
parameters $\bm{\beta}_{\mathrm{true}} = (1.0, 2.0, -1.0)^T$ reliably, and posterior
means are nearly indistinguishable between methods.

Variance collapse is present but mild: SD ratios stay above 0.97 for all three
coefficients, which is why this case is excluded from the Laplace correction analysis.
The intercept is marginally more affected than the slope --- the polynomial expansion
introduces additional between-predictor correlation --- but the effect is far weaker than
in the random-effects settings of CS4 and CS5.
...
\begin{figure}[H]
  ...
  \caption{Posterior comparison for $\beta_0$ (quadratic case): all four VI
           methods against the conjugate Gibbs reference.
           The approximation is slightly tighter than Gibbs (SD ratio above 0.97),
           consistent with the mild collapse visible across all three coefficients.}
...
\begin{figure}[H]
  ...
  \caption{Posterior comparison for the noise precision $\tau_e$ (quadratic case).
           The Gamma approximation tracks the Gibbs reference closely, with no
           meaningful collapse in the precision parameter.}
...
The model uses $\bm{\beta}_{\mathrm{true}} = (-1.0, 2.0)^T$ as data-generating
parameters; inference treats these as unknown and uses only the 200 binary observations
and the weakly informative prior $\bm{\beta}\sim\N(\bm{0},10\bm{I})$.
With this moderate sample size the prior retains some influence, pulling the posterior
mean to approximately $(-1.6,\;2.6)^T$ rather than landing on the true values.
The true parameters appear on the figures as a reference point --- they fall well within
the posterior, which confirms the sampler is correct.

Variance collapse is negligible in the logistic case, which is why CS3 is excluded from
the Laplace correction analysis.
The Jaakkola--Jordan bound is tight at the CAVI fixed point: when
$\xi_i^* = \sqrt{\E_q[(x_i^T\bm{\beta})^2]}$, the bound is tangent to the true
log-likelihood, so the ELBO penalises over-concentration nearly as strongly as the
exact posterior.
All four VI methods recover posterior standard deviations to within 3\% of the PG Gibbs
reference.
...
\begin{figure}[H]
  ...
  \caption{Posterior comparison for $\beta_0$ (logistic case): VI methods against the
           PG Gibbs reference.
           The posterior mean sits near $-1.6$ rather than the true value $-1.0$
           because the prior retains some influence at $n=200$.}
...
\noindent\textbf{AI tool used:} GitHub Copilot (Claude Sonnet~4.6).

\medskip
\noindent All mathematical derivations, algorithm design, experimental structure,
and intellectual content are the student's own work. AI was used for prose
editing, tactical execution of compiling \LaTeX, spell-checking, running code,
code debugging, and knowledge lookup as permitted by the ENEL445 course outline.

\medskip
\noindent The following prompts were submitted to the AI tool during preparation
of this report.

\begin{enumerate}
  \item Compile \texttt{20260515A-COMBINED-REPORT.tex} and commit to git.
  \item Are there any specific Python libraries relevant to what we are doing in these Python files?
  \item Is \texttt{ggplot} an R library?
  \item Is there any advantage to using \texttt{plotnine} --- is it prettier?
  \item At the beginning of the project, would it have been useful to have \texttt{plotnine}?
  \item Any other recommendations similar to the \texttt{polars} vs \texttt{pandas} choice?
  \item What is needed to fix the NumPy RNG to use the modern Generator API?
  \item In the course material, what is stated about the usage of LLMs and AI for the report?
  \item Construct a set of prompts that were used in this chat.
  \item Create an \texttt{.md} document labelled \texttt{prompts\_used.md} in the report folder and add these prompts to it.
  \item Recompile CS1, CS4, CS5 \LaTeX\ reports after overlay figures were added.
  \item These PDF files are in \texttt{results/pdf} --- confirm they exist.
  \item Their timestamps are around 10:30 --- verify.
  \item What does ``stuck in continuation mode'' mean?
  \item Why do I need to do anything for this to run? Is it not supposed to complete on its own?
  \item What is the $\sigma$ value used in the prior of CS1?
  \item Can it be smaller to make it still a flat prior?
  \item The Results and Discussion shows two images in the linear case --- prior and likelihood, then likelihood and posterior.
  \item In the image before it has prior and likelihood --- these are the initial conditions, is this not true?
  \item Are you saying that the prior should be in the second image as well?
  \item So what does this mean? [referring to the ``single panel'' claim]
  \item In the image the prior is seen as a straight line --- I don't see the reasoning that these densities have moved from the true value.
  \item Let's talk about the order of the report itself --- what is the order of CS1?
  \item Where CS1 is the baseline compared to CS4 and CS5.
  \item Do they match?
  \item Is there a good reason for the names of the sections in CS1 to be different?
  \item The prior is shown as a more horizontal line --- isn't this a density and wouldn't it have a vertical line for the prior at its expected value of zero?
  \item Where is the line in the Python code --- I want to convert it to a density.
  \item I have had too many failures within a chat and the context is lost --- create a handoff file summarising work done and what is left to do.
  \item Create a Markdown handoff file called \texttt{AGENT\_HANDOFF.md}.
  \item Create a file named \texttt{CONTINUE\_FROM\_CHECKPOINT.prompt.md}.
  \item Does it read the \texttt{.md} file you created for handoff?
  \item \texttt{Continue from checkpoint} created an \texttt{.md} file named what?
  \item I want to run a prompt to capture all pertinent information, then a new agent to read this information.
  \item Steps 1 and 2 yes --- would \texttt{AGENT\_HANDOFF} effectively be within one of these two?
  \item Please do [create \texttt{CAPTURE\_STATE.prompt.md} and rewrite \texttt{CONTINUE\_FROM\_CHECKPOINT.prompt.md}].
  \item [Ran \texttt{\#CAPTURE\_STATE.prompt.md} to generate \texttt{SESSION\_STATE.md}]
  \item What needs to be done about the terminals in \texttt{>>} state?
  \item In the report folder do you see a \texttt{.md} file?
  \item Add to it all prompts that were used in this chat.
  \item Read the MD files.
  \item What is Copilot's subscription cost for me?
  \item I thought Claude Code cost was similar to Copilot costs.
  \item This is the pricing I see. [screenshot: Claude Pro NZ\$34/month billed annually]
  \item Add a new appendix to the report for prompts used and include the content of \texttt{prompts-combined} in the report folder.
\end{enumerate}
