...
Throughout all gradient-based methods, the variational covariance $\bm{\Sigma}_\beta$ must remain symmetric positive definite at every 
iteration~\parencite[Example~3.7, p.~53]{Wainwright2008}.
\footnote{This structural constraint is satisfied automatically via Cholesky reparameterisation; see Appendix~\ref{app:cholesky}. For the role of symmetric positive definite covariance matrices in exponential families more broadly, see \textcite[Example~3.7, p.~53]{Wainwright2008}.} Standard gradient steps in Euclidean space can violate this requirement, leading to numerical failure. The solution is to parameterise the optimisation over the Cholesky factor $\bm{L}$ rather than $\bm{\Sigma}_\beta$ directly.
