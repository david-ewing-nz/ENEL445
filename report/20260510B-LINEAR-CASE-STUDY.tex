...
Mean-field Variational Inference (VI) provides a tractable approximation to Bayesian posterior computation, but systematically underestimates posterior uncertainty --- a failure known as \emph{posterior variance collapse}. This report investigates variance collapse in Bayesian linear regression using the gradient-based optimisation methods taught in ENEL445. Variational inference recasts posterior computation as maximisation of the Evidence Lower Bound (ELBO), making it a natural testbed for gradient ascent, Newton's method, and BFGS. Coordinate Ascent Variational Inference (CAVI) provides the analytical baseline, exploiting the conjugate Normal--Gamma structure of the model.
...
The ELBO is derived in closed form under a mean-field factorisation. The variational parameters are $\bm{\mu}_\beta$, $\bm{\Sigma}_\beta$, $a_e$, and $b_e$; Cholesky and log-space reparameterisations keep the covariance matrix positive definite and the Gamma parameters positive throughout optimisation. For each gradient-based method, the report states the search direction, step-size rule, and exit conditions. Line-search methods require the ascent condition
\begin{align*}
\nabla\ELBO(\bm{\phi}^{(t)})^T \bm{d}^{(t)} > 0;
\end{align*}
gradient ascent uses Armijo backtracking and BFGS uses Wolfe conditions to maintain the curvature condition $\bm{y}_t^T\bm{s}_t>0$ for the inverse-Hessian update.

The methods are compared on ELBO convergence, computational cost, posterior mean accuracy, and posterior standard-deviation ratios against a Gibbs sampler reference. A secondary comparison uses an entropy-regularised ELBO to examine whether explicitly incentivising broader posteriors can reduce collapse.
\end{abstract}
...
compounded by the mean-field factorisation assumption.\footnote{See Appendix~\ref{app:vb-collapse} for the full mechanistic explanation.}

This paper applies four optimisation methods---CAVI, gradient ascent, Newton's method, and BFGS---to the ELBO and compares their convergence behaviour, computational cost, and posterior quality.
...
Because this case study uses conjugate Gaussian--Gamma priors, the true posterior $p(\bm{\beta}, \tau_e \mid \bm{y})$ is analytically available as a Normal--Gamma distribution. This makes this case study the only one in this project where posterior variance collapse can be measured against ground truth, not merely against a sampler reference. The Gibbs sampler still serves as a practical reference, but the true posterior provides an exact benchmark against which both the VI approximation and the sampler can be validated.

Posterior variance collapse refers to the tendency of mean-field VI to produce posterior approximations with variances that are too small. In Bayesian regression, this appears as a covariance matrix $\bm{\Sigma}_\beta$ whose diagonal entries underestimate the uncertainty in the regression coefficients. The resulting posterior intervals are too narrow, making the model appear more confident than it should be.

Convergence speed and ELBO value are necessary measures of optimiser performance, but they are not sufficient. An optimisation method may converge reliably and achieve a high ELBO while still producing a posterior approximation that understates uncertainty---and in Bayesian inference, calibrated uncertainty is a primary concern, not an afterthought. Both convergence quality and posterior dispersion are evaluated in this report.

\subsection{Why Collapse Occurs}

Mean-field VI restricts the approximating family by assuming
\begin{align}
q(\bm{\beta},\tau_e) = q(\bm{\beta})\,q(\tau_e),
\end{align}
which forces independence between the regression coefficients and the noise precision. In this case study the true Normal--Gamma posterior contains genuine dependence between $\bm{\beta}$ and $\tau_e$: a larger noise precision implies tighter coefficient estimates, so the two are correlated in the exact posterior. The mean-field factorisation removes this dependence by assumption, discarding uncertainty that would otherwise appear through the joint structure.
...
\end{align}
This objective strongly penalises $q$ for placing mass where the true posterior is small, but does not penalise $q$ for failing to cover regions where the true posterior is large. The result is that $q$ locks onto the high-probability core of $p(\bm{\beta},\tau_e\mid\bm{y})$ and ignores the tails, systematically underestimating posterior variance. Because the true posterior is available analytically in this case study, this effect can be quantified exactly rather than inferred from samples.
...
The following defines the test case and the data-generation code used in the experiments. The full implementation structure---ELBO evaluation, optimisers, and Gibbs sampler---is in Appendix~\ref{app:python-code}.
...
Each method is assessed against three criteria. First, convergence: whether the method reaches the same ELBO optimum as CAVI, which provides the analytical fixed point for this conjugate model. Second, posterior accuracy: whether the resulting variational posterior matches the Gibbs sampler and, in this case study uniquely, the analytically available true posterior. The Gibbs sampler is the gold-standard uncertainty reference; it is not an ELBO optimiser and is not compared on convergence speed. Third, computational cost: wall-clock time and iteration count. Analytical gradients are verified against central finite differences to relative error $\leq 10^{-5}$ before any optimiser is run.
...
Four strategies exist for handling the constraints $\bm{\Sigma}_\beta \succ 0$ and $a_e, b_e > 0$ in a gradient-based optimiser.
\begin{enumerate}
...
\end{enumerate}
This work uses reparameterisation throughout: it requires no tuning parameters, no outer loops, and produces a smooth unconstrained problem fully compatible with gradient ascent, Newton, and BFGS.
