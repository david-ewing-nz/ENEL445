...
Bayesian logistic regression is the simplest non-trivial model in which the posterior is
\underline{genuinely non-conjugate} and no component of variational inference---the ELBO, its gradient,
or the posterior approximation---admits a closed-form expression, making it the natural
test case for methods that must navigate an analytically intractable objective.
...
\end{equation*}
where $p(\bm{y} \mid \bm{\beta}) = \prod_{i=1}^{n} \mathrm{Bernoulli}(y_i;\,\sigma(x_i^T\bm{\beta}))$
is the logistic likelihood and $\sigma(\cdot)$ is the sigmoid function.
...
approximation underestimates posterior uncertainty;\footnote{See \ref{app:vb-collapse} for the full mechanistic explanation.} this is assessed empirically in the Results section.
...
\end{equation}
At the optimal $\xi_i^* = \sqrt{\E_q[\psi_i^2]}$\footnote{See \ref{app:jj-bound} for the derivation of the tightness condition and the origin of $\tanh(\xi/2)$.}, where
...
\newpage
\section{Jaakkola--Jordan Bound: Tightness Condition}
\label{app:jj-bound}
%─────────────────────────────────────────────────────────────────────────────

The Jaakkola--Jordan bound replaces the intractable log-sigmoid term with a quadratic
lower bound parameterised by $\xi_i > 0$\,\parencite{Jaakkola2000}:
\begin{equation*}
  f(\psi_i, \xi_i) = \frac{\psi_i}{2} - \log 2\cosh\!\left(\frac{\xi_i}{2}\right)
  - \frac{\tanh(\xi_i/2)}{4\xi_i}\bigl(\psi_i^2 - \xi_i^2\bigr).
\end{equation*}
This is tight when $\psi_i = \xi_i$, i.e.\ the bound touches $\log\sigma(\psi_i)$ at
$\psi_i = \pm\xi_i$.

\textbf{Origin of $\tanh(\xi/2)$.}
Differentiating $f(\psi, \xi)$ with respect to $\xi$ and setting to zero:
\begin{align*}
  \frac{\partial f}{\partial \xi}
  &= -\frac{1}{2}\cdot\frac{\sinh(\xi/2)}{\cosh(\xi/2)}
     - \frac{\partial}{\partial\xi}\!\left[\frac{\tanh(\xi/2)}{4\xi}\right](\psi^2-\xi^2)
     + \frac{\tanh(\xi/2)}{4\xi}\cdot 2\xi = 0.
\end{align*}
At $\psi = \xi$ the residual term $\psi^2 - \xi^2 = 0$ and the condition reduces to
$\tanh(\xi/2)/2 = \tanh(\xi/2)/2$, confirming tightness.
The coefficient $\tanh(\xi/2)/(4\xi)$ is denoted $\lambda(\xi)$ in \textcite{Jaakkola2000}
and arises from the second derivative of $\log\cosh(\xi/2)$:
\begin{equation*}
  \lambda(\xi) = \frac{\tanh(\xi/2)}{4\xi}.
\end{equation*}

\textbf{Tightness condition.}
Taking the expectation over $q(\bm{\beta}) = \mathcal{N}(\bm{\mu},\bm{\Sigma})$,
the bound is tightest when $\xi_i$ matches the root-mean-square of $\psi_i = x_i^T\bm{\beta}$:
\begin{equation*}
  \xi_i^* = \sqrt{\E_q[\psi_i^2]} = \sqrt{(x_i^T\bm{\mu})^2 + x_i^T\bm{\Sigma}x_i}.
\end{equation*}
This follows because the bound $f(\psi_i,\xi_i)$ is maximised over $\xi_i$ when
$\xi_i^2 = \E_q[\psi_i^2]$, i.e.\ the parameter $\xi_i$ is set equal to the
expected magnitude of the linear predictor under $q$.
