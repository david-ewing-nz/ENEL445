...
A Comparative Study Across Four Bayesian Regression Settings
}
\author{
...
inference across four Bayesian regression settings: linear, quadratic, logistic,
...
and the hierarchical linear case adds group random effects ($J=5$ groups, $N=150$, $p=2$).
Posterior variance collapse is mild in the linear case, moderate in the quadratic case,
minimal in the logistic case, and mild in the hierarchical case.
CAVI is the most efficient method across all four settings.
...
This project applies the optimisation methods taught in ENEL445 to four Bayesian
...
\begin{description}
  ...
  \item[Case Study 4 --- Hierarchical Linear Regression:]
    Two-level Gaussian model with $J=5$ group random effects.
    ELBO dimension $D=19$; mean-field approximation over $\bm{\beta}$, $u_j$, $\tau_e$, $\tau_u$.
    Three-chain Gibbs sampler as the reference.
    ($N=150$, $J=5$, $\bm{\beta}_{\mathrm{true}} = (1.0, 2.0)$.)
\end{description}

All four studies share the same four optimisation methods (CAVI, gradient ascent,
...
uncertainty.
This failure mode is observed and analysed across all four settings.
...
\begin{table}[H]
\centering
\caption{Comparison of the four case studies.}
\label{tab:comparison}
\begin{tabular}{@{}lllll@{}}
\toprule
Property & Linear & Quadratic & Logistic & Hierarchical \\
\midrule
Likelihood & Gaussian & Gaussian & Bernoulli & Gaussian \\
True $\bm{\beta}$ & $(1.0, 2.0)$ & $(1.0, 2.0, -1.0)$ & $(-1.0, 2.0)$ & $(1.0, 2.0)$ \\
Predictors $p$ & 2 & 3 & 2 & 2 \\
Observations $n$ & 50 & 100 & 200 & 150 ($J\!=\!5$ groups) \\
Conjugacy & Yes & Yes & No (JJ bound) & Yes \\
ELBO form & Normal--Gamma & Normal--Gamma & Jaakkola--Jordan & Normal--Gamma \\
Reference sampler & Conjugate Gibbs & Conjugate Gibbs & PG Gibbs & Hier.~Gibbs \\
Gibbs cost & Low & Low & High ($n \times T$) & Medium \\
Vartnl.~parameters & $p + p(p+1)/2 + 2$ & $p + p(p+1)/2 + 2$ & $2p + 1$ & $D = 19$ \\
Variance collapse & Mild & Moderate & Minimal & Mild \\
CAVI updates & Closed form & Closed form & Closed form (via JJ) & Closed form \\
\bottomrule
\end{tabular}
\end{table}
...
uncertainty --- varies systematically across the four cases.
...
In the hierarchical case, the mean-field factorisation separates $q(\bm{\beta})$
from $q(u_j)$ and both from $q(\tau_u)$, removing correlations between the fixed
effects and the random-effect precision.
The resulting variance collapse is mild, similar to the simple linear case,
because the within-group structure ($n_j \approx 30$) provides strong local information.

\subsection{Optimisation Behaviour}

CAVI is the most efficient method across all four settings, because it exploits
closed-form coordinate updates.
For the linear, quadratic, and hierarchical cases, the conjugate Normal--Gamma
structure enables exact updates; for the logistic case, the JJ bound restores the
same conjugate structure.

BFGS converges rapidly in all four cases, providing an effective gradient-based
...
case ($D=19$), the Hessian cost is higher and the method exhausts its iteration budget
without fully converging, making BFGS the preferred second-order alternative.
...
across four Bayesian regression settings of increasing difficulty.
...
Across all four case studies, CAVI is the most efficient method, converging rapidly
...
The four cases expose a clear progression:
\begin{enumerate}
  ...
  \item The hierarchical case adds group random effects ($J=5$, $D=19$), demonstrating
    that the same mean-field VI framework scales to mixed-effects models with minimal
...
True $\bm{\beta}$ & $(1.0, 2.0)$ & $(1.0, 2.0, -1.0)$ & $(-1.0, 2.0)$ & $(1.0, 2.0)$ \\
Predictors $p$ & 2 & 3 & 2 & 2 \\
Observations $n$ & 50 & 100 & 200 & 150 ($J\!=\!5$) \\
Conjugacy & Yes & Yes & No & Yes \\
\midrule
\multicolumn{5}{@{}l}{\textbf{Variational approximation}} \\
Variational family & Normal--Gamma & Normal--Gamma & Gaussian only & Normal--Gamma \\
ELBO form & Closed form & Closed form & Jaakkola--Jordan & Closed form \\
VI parameters $d$ & 8 & 11 & 5 & 19 \\
Variance collapse & Mild & Moderate & Minimal & Mild \\
\midrule
\multicolumn{5}{@{}l}{\textbf{Reference sampler}} \\
Sampler type & Conjugate Gibbs & Conjugate Gibbs & PG Gibbs & Hier.~Gibbs \\
Cost per sweep & $O(p^3)$ & $O(p^3)$ & $O(n \cdot T)$ & $O(N + J)$ \\
ESS (typical) & High & High & $\approx 250$--$280$ & High \\
\midrule
\multicolumn{5}{@{}l}{\textbf{Optimisation}} \\
CAVI convergence & $<50$ iter & $<50$ iter & $<50$ iter & $\approx 100$ iter \\
BFGS convergence & Fast & Fast & Fast & Fast \\
Recommended method & CAVI & CAVI & CAVI & CAVI \\
\bottomrule
\end{tabular}
\end{table}
...
The four individual case study reports contain full derivations, all figures, and
...
\begin{description}
  ...
  \item[Hierarchical linear regression:] \texttt{python/hierarchical\_linear\_run.ipynb}.
    \label{app:hier-ref}
    Two-level Normal--Gamma ELBO, group random effects, three-chain Gibbs sampler,
    and all 23 result figures.
\end{description}
...
Hierarchical  & \texttt{python/hierarchical\_linear\_run.ipynb} & \texttt{hierarchical\_linear\_} \\
\bottomrule
\end{tabular}
\end{center}
