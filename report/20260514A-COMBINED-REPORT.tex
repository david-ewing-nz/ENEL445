...
\begin{description}
  ...
  \item[Case Study 5 --- Hierarchical Logistic Regression:]
    Two-level logistic model combining the Jaakkola--Jordan ELBO with group random effects.
    ELBO dimension $D=17$; P\'{o}lya--Gamma Gibbs sampler as the reference.
    Symmetric posterior variance collapse across both coefficients.
    ($N=150$, $J=5$, $\bm{\beta}_{\mathrm{true}} = (-1.0, 2.0)$.)
\end{description}

All five studies share the same four optimisation methods (CAVI, gradient ascent,
...
computational cost, posterior accuracy, and variance collapse behaviour across
all five settings.
...
across five Bayesian regression settings of increasing difficulty.
...
Across all five case studies, CAVI is the most efficient method, converging rapidly
...
\begin{enumerate}
  ...
  \item The hierarchical linear case adds group random effects ($J=5$, $D=19$),
    demonstrating that the same mean-field VI framework scales to mixed-effects models
    with minimal additional modelling effort. Asymmetric variance collapse emerges
    from the entanglement of $\beta_0$ with the random effects.
  \item The hierarchical logistic case combines the JJ bound with random effects
    ($J=5$, $D=17$), completing the progression. Variance collapse becomes symmetric
    across both coefficients, and PG Gibbs confirms the VI solution.
\end{enumerate}

The P\'{o}lya--Gamma Gibbs sampler provides an accurate reference for the logistic
and hierarchical logistic cases but is computationally expensive ($n \times T$
Gamma draws per iteration).
For production settings with $n \gg 200$, the VI methods offer a substantial
computational advantage --- CAVI is 1050$\times$ faster than PG Gibbs in Case Study~5.
...
\begin{description}
  \item[CS1 --- Linear regression:] \texttt{report/20260510A-LINEAR-CASE-STUDY.tex}.
  ...
  \item[CS2 --- Quadratic regression:] \texttt{report/20260510A-QUADRATIC-CASE-STUDY.tex}.
    Identical framework to the linear case with $p=3$ and feature expansion.
  \item[CS3 --- Logistic regression:] \texttt{report/20260510A-LOGISTIC-CASE-STUDY.tex}.
    Full Jaakkola--Jordan derivation, P\'{o}lya--Gamma sampler, ESS diagnostics,
    and all 20 result figures.
  \item[CS4 --- Hierarchical linear regression:] \texttt{report/20260510A-HIERARCHICAL-LINEAR-CASE-STUDY.tex}.
  ...
  \item[CS5 --- Hierarchical logistic regression:] \texttt{report/20260510A-HIERARCHICAL-LOGISTIC-CASE-STUDY.tex}.
