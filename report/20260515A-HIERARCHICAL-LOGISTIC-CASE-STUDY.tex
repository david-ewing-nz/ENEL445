...
Figure~\ref{fig:likpost} overlays the CAVI posterior group-level curves on the data.
The prior mean for $\boldsymbol{\beta}$ is $\mathbf{0}$, which predicts a flat
sigmoid at $P(y=1)=0.5$ for all $x$.
The likelihood shifts the posterior toward the group-specific data, pulling
$\hat{\beta}_1 \approx 2.2$ away from the prior mean of zero and separating
the group curves according to their random effects $\hat{u}_j$.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{hierarchical_logistic_likelihood_posterior.png}
  \caption{CAVI group posterior logistic curves (coloured dashed) and population
    posterior mean (steelblue, with 95\,\% CI on the linear-predictor scale).
    Black dashed: true population curve.
    Shaded bands show $\pm 2$ posterior SD propagated through the logistic link.}
  \label{fig:likpost}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{hierarchical_logistic_likelihood_posterior_gibbs.png}
  \caption{Population-level 95\,\% credible intervals: CAVI (steelblue) vs Gibbs MCMC (tomato),
    overlaid on the per-group CAVI sigmoid bands.
...
\begin{table}[H]
  ...
  \label{tab:perf}
  \input{../results/tables/hierarchical_logistic_performance.tex}
\end{table}
...
\begin{table}[H]
  ...
  \label{tab:post}
  \input{../results/tables/hierarchical_logistic_posterior_summary.tex}
\end{table}
