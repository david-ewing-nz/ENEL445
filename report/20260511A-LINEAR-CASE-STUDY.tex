...
I apply four optimisation methods---CAVI, gradient ascent, Newton's method, and BFGS---to the ELBO and compare their convergence behaviour, computational cost, and posterior quality.
...
To introduce the objective pedagogically, I start from two compact identities:

\begin{align}
\mathrm{KL}(q\|p)
&= \underbrace{-H[q]}_{\text{negative entropy of }q}
  - \underbrace{\E_q[\log p(\bm{\beta})]}_{\text{expected log-prior}},
\end{align}

\begin{align}
\mathcal{L}(\theta)
&= \underbrace{\E_q[\log p(\bm{y}\mid\bm{\beta})]}_{\text{fit the data}}
  - \underbrace{\mathrm{KL}(q\|p)}_{\text{stay close to prior}}.
\end{align}
...
Step 3: in this report, these two ideas are written in joint-form ELBO notation over $(\bm{\beta},\tau_e)$, which is the objective optimised by all methods.
...
Outside this report context, however, that visual overlap could be misread as posterior collapse, so the figure should be interpreted together with the SD-ratio and Gibbs comparisons.
...
I used Bayesian linear regression as a checked setting for comparing CAVI, gradient ascent, Newton's method, and BFGS on the same variational inference problem. The formulation is an ELBO maximisation problem, but the numerical question is broader than maximisation alone.
