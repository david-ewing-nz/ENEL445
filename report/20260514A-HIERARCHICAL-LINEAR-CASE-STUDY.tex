...
\begin{equation*}
    \mathcal{L}_{\mathrm{reg}}(\bm{\theta}) = \mathcal{L}(\bm{\theta}) + \lambda H[q(\bm{\beta})],
\end{equation*}
...
Writing the likelihood scalar form makes the two-level structure explicit:
\begin{equation}
  y_{ij} = \underbrace{\beta_0 + \beta_1 x_{ij}}_{\text{population mean}}
          + \underbrace{u_j}_{\text{group shift}}
          + \varepsilon_{ij}, \qquad
  \varepsilon_{ij} \sim \mathcal{N}(0,\,\tau_e^{-1}).
\end{equation}
The \emph{population mean} $\beta_0 + \beta_1 x_{ij}$ is shared across all groups;
the \emph{group shift} $u_j$ is a group-specific offset drawn from
$\mathcal{N}(0, \tau_u^{-1})$, inducing within-group correlation.
Observations within the same group $j$ are therefore not independent --- they
share the same realisation of $u_j$ --- which is the defining characteristic of
...
\subsection{Test Case: Hierarchical Bayesian Linear Regression}
...
\begin{figure}[H]
  ...
  \caption{Prior and likelihood for the hierarchical linear model ($N=150$, $J=5$).
    The grey band is the prior 95\% predictive interval (clipped) for the population mean
    under $\bm{\beta}\sim\mathcal{N}(\mathbf{0},10\mathbf{I})$; the grey dashed line is
    the prior mean $y=0$.
    Coloured bands show the per-group likelihood 95\% noise interval
    $\pm 2\sigma_e = \pm 1.0$ around each group's true regression line
    $\beta_0 + \beta_1 x + u_j$; dotted lines are the group regression means.
    The black dashed line is the population mean $\beta_0 + \beta_1 x$.}
  \label{fig:data}
\end{figure}

Figure~\ref{fig:likpost} shows the CAVI posterior bands for each group alongside the
population posterior.
Because each observation belongs to exactly one group, the observed $y_{ij}$ scatters
around its \emph{group mean}:
\begin{equation}
  y_{ij} \mid \boldsymbol{\beta}, u_j, \tau_e
    \;\sim\; \mathcal{N}\!\Bigl(
      \underbrace{\beta_0 + \beta_1 x_{ij}}_{\text{population mean}}
      +\underbrace{u_j}_{\text{group shift}},\;
      \underbrace{\tau_e^{-1}}_{\text{within-group variance}}
    \Bigr).
\end{equation}
Each coloured band in Figure~\ref{fig:likpost} shows the CAVI posterior uncertainty
$\pm 2\sqrt{v_{\beta}(x)+v_j}$ around the fitted group mean, where $v_j$ is the
posterior variance of $u_j$ and $v_{\beta}(x) = \mathbf{x}^\top \boldsymbol{\Sigma}_\beta \mathbf{x}$
is the posterior variance from $\boldsymbol{\beta}$.
Because CAVI underestimates both quantities under the mean-field independence assumption,
the resulting bands are visibly narrower than the actual data scatter, directly
illustrating variance collapse.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{hierarchical_linear_likelihood_posterior.png}
  \caption{Per-group CAVI posterior bands and population posterior for the hierarchical
    linear model.
    Each coloured dashed line is the CAVI posterior group mean
    $\hat{\beta}_0 + \hat{\beta}_1 x + m_j$ (same group colours as
    \cref{fig:datascatter}); the shaded band shows $\pm 2\sqrt{v_{\beta}(x)+v_j}$,
    combining posterior uncertainty in $\boldsymbol{\beta}$ with posterior uncertainty
    in the group effect $u_j$.
    The blue band is the corresponding population-level posterior
    $\pm 2\sqrt{v_{\beta}(x)}$, containing $\boldsymbol{\beta}$ uncertainty only.
    The black dashed line is the true population mean $y=1.0+2.0x$.
    Two symptoms of variance collapse are immediately visible: (1)~the posterior bands
    are far narrower than the data scatter warrants --- many observations fall outside
    their group's posterior band --- indicating that $\boldsymbol{\Sigma}_\beta$ and
    $v_j$ are jointly underestimated; and (2)~the population posterior mean
    ($\hat{\beta}_0 \approx 0.46$) lies below the true population mean by
    $\approx 0.54$ units, reflecting intercept bias induced by the mean-field
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{hierarchical_linear_likelihood_posterior_gibbs.png}
  \caption{Population-level 95\,\% credible intervals: CAVI (steelblue) vs Gibbs MCMC (tomato),
    overlaid on the per-group CAVI posterior bands.
    The Gibbs CI is substantially wider than the CAVI CI for the same population mean,
    directly demonstrating posterior variance collapse under the mean-field approximation.}
  \label{fig:likpost_gibbs}
\end{figure}
...
\begin{table}[H]
  ...
  \label{tab:perf}
  \input{../results/tables/hierarchical_linear_performance.tex}
\end{table}
...
\begin{table}[H]
  ...
  \label{tab:post}
  \input{../results/tables/hierarchical_linear_posterior_summary.tex}
\end{table}
