...
The linear test case is the most favourable setting for mean-field VI.
The true posterior is unimodal and nearly Gaussian, so the mean-field factorisation
$q(\bm{\beta},\tau_e) = q(\bm{\beta})\,q(\tau_e)$ loses little information.
CAVI exploits the conjugate structure to find the optimal mean-field approximation
in closed form, achieving SD~ratios of 0.98--0.98 — close to one despite the
factorisation assumption.

The gradient-based methods reveal two distinct failure modes. First, Newton's Method
and BFGS converge to local optima that are substantially worse than the CAVI solution,
demonstrating that generic optimisers without analytical gradients can fail even
on a convex ELBO landscape. Second, Gradient Ascent with finite-difference gradients
offers no speed advantage over Gibbs on small problems: the $O(2d)$ function evaluations
per gradient step dominate the total cost at $d=7$ unconstrained parameters.

These results motivate two conclusions for the broader study. First, when conjugacy
is available, CAVI is the optimal strategy and the generic gradient methods cannot
improve upon it. Second, when conjugacy is unavailable (non-conjugate likelihoods,
deeper models), gradient-based ELBO optimisation becomes the only viable VI route ---
but analytical gradients (via automatic differentiation) will be essential to recover
competitive runtime.
